...
pub type BoxCoroutine = Box<dyn Unpin + DeviceCoroutine>;
...
// inner coroutine's code (yields included) into the caller. This codebase uses macros
// in some places to reduce both code duplication and coroutine dispatch costs.
macro_rules! yield_all {
    ($coroutine_expr:expr) => {{
        let mut coroutine = $coroutine_expr;
        loop {
...
// Run the coroutine until it is exhausted, discarding yields.
